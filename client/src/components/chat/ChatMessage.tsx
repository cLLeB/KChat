...
export function ChatMessage({ message, isOwn, onImageView }: ChatMessageProps) {
  ...
  const [secondsLeft, setSecondsLeft] = useState<number | null>(() => {
    ...
    const expires = new Date(message.expiresAt as any);
    const diff = Math.max(0, Math.round((expires.getTime() - Date.now()) / 1000));
    return diff;
  });
        ...
        <div className="space-y-1 flex-1">
          ...
          <div className={cn(
            ...
            {secondsLeft !== null && secondsLeft >= 0 && (
              <div className="ml-2 px-2 py-0.5 rounded bg-amber-50 text-amber-700 text-[10px]">{secondsLeft}s</div>
            )}
